...
Applying §1.2: LÖVE's own API and the browser platform are **this
repository's subject** — `love.graphics`, `Canvas`, WebGL2, `requestAnimationFrame`
are used bare. The wasm and WASI toolchain machinery is **borrowed** — wasm-EH,
`setjmp`/`longjmp` lowering, reactor exec model, preopens get defined at first
use, every time they cross a document boundary.
...
| The claim | What earns it |
|---|---|
| An engine module works over its seam | A witness driving the real module, passing on every leg it can reach |
| A capability the browser lacks is handled | A witness asserting the refusal is catchable and the module survives it |
| A number about the build or the runtime | A measurement citing its run (§5) |

A witness runs every engine available to it — `node:wasi` and real Chromium,
plus real Firefox where the check is engine-sensitive — and **every leg must
pass**. A witness that cannot run on a leg says so and why, in the script's own
...
committed `testing/**/expected/` outputs are how we reach it without running
it. Where no desktop reference exists — the browser seams themselves have no
counterpart — the witness is written from intent, and its header says so,
because it is then the specification.

### 7.5 Divergences are declared, never faked

Where the browser cannot provide a capability faithfully, the seam **reports it
unsupported** rather than emulating it. Where the call must stay non-fatal, its
first use emits a one-time `[love.wasm preview]` notice and returns a safe
default. Emulation that quietly returns a plausible answer teaches the wrong
model and is the one thing §2.4 forbids outright.

### 7.6 Shared engine source is upstream's

`src/**` is upstream's code. Changes there are small **guarded seams**:
byte-unchanged for default builds, shaped to be offered upstream. A
fork-private behavior change to shared engine source is not allowed — if
upstream declines something this port needs, that becomes a recorded decision
here, never a silent divergence. Exclusion happens in the build, never with
`rm`, so the tree stays upstream-shaped and rebases stay cheap.

### 7.7 Toolchain invariants

Every build needs `PREFIX` pointing at a wasm-EH sysroot, and every
translation unit carries the *standardized* wasm-EH encoding — the compiler
default is the legacy one, so the flag is explicit and gated per artifact by
`wasi/toolchain/check-eh-encoding.sh`. The LLVM major and the EH encoding must
match [lua.wasm][luawasm]'s, because the VM and this engine share one
exception-handling machinery. These are invariants, not preferences: a lost
flag produces an artifact that links and then misbehaves, which is why the gate
exists rather than a convention.

[upstream]: https://github.com/love2d/love
[luawasm]: https://github.com/andy-emerson/lua.wasm
