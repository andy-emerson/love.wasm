...
| Filesystem / `.love` mounting | PhysFS | host-import VFS (`love_fs`) backed by the host's project storage, with `t.identity` namespacing preserved. Read, write and enumerate are real: a separate writable save namespace sits beside the read-only project, OPFS-backed in the browser (D2), and reads resolve save-first so a written file shadows a project file without mutating it. Runtime archive/`.love` mounting is a **declared divergence** — D7 closed as deliberately not built (#48); a `.love` as the *boot source* needs it anyway, since the seam takes files and a host unzips in its own JS |
...
Each runs every engine it can reach, and every leg must pass. A Chromium-only witness is one that needs a real WebGL2 context, which node has no equivalent for. All of them run in CI on every push or pull request to `wasi` (`.github/workflows/witness.yml`) — except `wasi/games/run.sh`, which is on demand by design, and `wasi/corpus/run.sh`, whose workflow step is written but not yet committed (see **Corpus parity** above).
