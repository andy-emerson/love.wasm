...
**Status: the LÖVE core boots in a real browser, plays and captures audio, and now clears a real WebGL2 framebuffer through `love.graphics`.** Build-order steps 0–3 and 5 are done and browser-verified (see the build order below): real `love.math` and `love.data` run under this repo's frame pump over the pinned lua-wasi source drop, LÖVE 12's own main-loop function executes to its documented stop-line at the `love.filesystem` seam, and `love.audio` — playback and microphone, on a WebAudio backend — is witnessed recovering a test tone headless. Step-4 sub-step 4.1 has since landed: `love.graphics` on the **reused** `opengl` backend (only its GL loader reseamed to static WebGL2 imports), with real glslang shader reflection and real FreeType/HarfBuzz linked, brings up its pipeline and clears a real framebuffer in Chromium with the cleared colour recovered exactly — clear-only so far, actual drawing is 4.2+. The tree stays upstream-shaped — edits to shared engine source are three small guarded seams, each byte-unchanged for default builds and offered upstream: the platform seam in `common/config.h` (a three-line `__wasi__` → `LOVE_WASI` guard plus one clause in the platform sanity-check), the audio-backend-selection seam in `src/modules/audio/wrap_Audio.cpp` (OpenAL/WebAudio/null), and the GL-loader seam in `src/modules/graphics/opengl/OpenGL.cpp` (static WebGL2 imports vs the SDL proc-address loader). Two further portability fixes to the vendored `glslang` are carried lane-2 patches (offered upstream), not fork seams. Outside `wasi/`, the fork adds only two CI workflows (`.github/workflows/witness.yml` and `.github/workflows/publish-sysroot.yml`), the interactive-session bring-up hook and settings under `.claude/`, a few `.gitignore` lines, and the governance docs (`AGENTS.md`, `CLAUDE.md`).
...
1. **Graphics/window.** Upstream: SDL3 provides window/context; `graphics/opengl` issues GL calls. Here: no SDL, no EGL. Because WebGL2 *is* OpenGL ES 3.0 — which LÖVE's `opengl` backend already targets — the backend is **reused, not rewritten**: only its GL loader is reseamed to **static WebGL2 imports** (a guarded `LOVE_GRAPHICS_GL_STATIC_IMPORTS` seam in `OpenGL.cpp`, byte-unchanged for default builds and offered upstream), so the host's WebGL2 context is the GL surface. **Sub-step 4.1 landed:** `love.graphics` brings up its pipeline under host-reported limits and clears a real framebuffer in Chromium, the cleared colour recovered exactly (clear-only; drawing is 4.2+). `love.window` is a step-6 seam, stubbed for now.
...
The CI-enforced witnesses (`wasi/{witness,pump,boot}/run.sh`) are one-command each and run on every push to `wasi` (`.github/workflows/witness.yml`; every leg — `node:wasi` and real Chromium, plus a wasmtime leg on the EH witness — must pass, alongside the vendored-dependency witnesses). The later seams add their own one-command witnesses — `wasi/audio/run.sh` (step 5) and `wasi/graphics/run.sh` (step 4.1) — green by hand but not yet CI-wired. They are also reproducible in an interactive session: `.claude/hooks/session-start.sh` brings the toolchain up on session start — `clang-20` from Ubuntu's own repos, Node ≥ 24.15 from the `nodejs.org` tarball (the web sandbox denies `apt.llvm.org` and `deb.nodesource.com`), `playwright-core` over the pre-provisioned Chromium — and fetches the prebuilt wasm-EH sysroot published by `.github/workflows/publish-sysroot.yml`, so the witnesses pass green in-session without a minutes-long from-source sysroot rebuild in every ephemeral container. This is what lets the remaining seams (steps 4–8) be driven and witnessed by hand, not just in CI.
