...
**Modules a game may use.** LÖVE enables all twenty modules by default and `boot.lua` hard-errors on a missing one, so a build shipping a subset has to answer for the rest. The union artifact links eighteen — everything except `touch`, `video` and `thread`. Those three are supplied by the boot wrapper (`wasi/platform/witness-frame.lua`) as absent modules: `require` succeeds so a game boots, `love.<name>` stays `nil` exactly as it is on desktop with `t.modules.<name> = false`, and reading it emits one `[love.wasm preview]` notice naming the module. A game therefore needs no `conf.lua` edit to boot, and a game that genuinely uses one of the three says so in the log rather than failing anonymously.
...
| Filesystem / `.love` mounting | PhysFS | host-import VFS (`love_fs`) backed by the host's project storage, with `t.identity` namespacing preserved. Read, write and enumerate are real: a separate writable save namespace sits beside the read-only project, OPFS-backed in the browser (D2), and reads resolve save-first so a written file shadows a project file without mutating it. Archive/`.love` mounting is declared-unimplemented pending an open decision — who unzips a runtime-mounted archive, host JS or a guest zip reader over the in-tree zlib |
...
| 8 | Packaging | **Not started.** Single `.js` with the wasm embedded versus a `.js` + `.wasm` pair, decided by measuring cold and warm load rather than by argument (#7) |

Modules with no faithful browser primitive are declared, not faked: networking (`enet`/`luasocket`/`luahttps`) is absent, `love.video` dropped Theora because CPU-decoding it single-threaded would stutter — lower fidelity than an honest notice — and archive/`.love`-zip mounting stays enumeration-only while an open decision (who unzips: host JS or a guest zip reader over the in-tree zlib) waits for a game that needs it.
...
Each runs every engine it can reach, and every leg must pass. A Chromium-only witness is one that needs a real WebGL2 context, which node has no equivalent for. All of them run in CI on every push or pull request to `wasi` (`.github/workflows/witness.yml`).
