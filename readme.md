...
**Corpus parity — where it actually stands.** The `testing/` conformance corpus now runs under this build: **303 pass / 37 fail / 15 skip** across 21 suites, up from 236/92 on its first run. Every failure has been examined and classified in `wasi/COMPATIBILITY.md`; **30 are features a browser does not have**, 4 need a user gesture no test can supply, and **3 are real gaps** — the Screen Wake Lock (unbuilt) and DXT1 (#51). What still stands between here and Beta is making that classification executable, so the corpus fails loudly both when a passing test breaks and when a divergence quietly starts working.
