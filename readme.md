...
**Branch model.** **`main` is the trunk and the repo default** — everything this project adds lives there, reviewed and witnessed. **`upstream-mirror`** is a pristine copy of upstream `love2d/love` at the base pin above — never committed to, only advanced deliberately (through an explicit `upstream` remote) when a new upstream base is adopted. At any moment, `git diff upstream-mirror...main` is the complete, current answer to "what did this fork touch" — the evidence for the honest claim above. (The repository is detached from the GitHub fork network as of 2026-08-11; the former trunk `wasi` is retired, its history fully contained in `main`.)

**Three lanes for changes:**

1. **Wasm-only code** — new backends, build files, the pump. Additive by design (new files, minimal edits to shared code), lives on `main` forever, rebases near-conflict-free.
2. **Generic fixes LÖVE needs** — cut a topic branch from the *upstream base commit* (not from `main`), PR it to love2d from a fresh fork, cherry-pick it into `main` meanwhile. When upstream merges, the carried copy is shed automatically at the next rebase (git recognizes already-applied patches). Carried, never kept.
...
**The swap.** Adopting a new upstream — a fresh nightly or the 12.0 release itself — is: `git fetch upstream`, advance `upstream-mirror` to the new commit, rebase `main`'s fork-added history onto it, watch lane-2 patches fall out, update the base pin at the top of this README. Because lane 1 is additive and lane 2 self-sheds, the expected cost is an afternoon, not a migration.
