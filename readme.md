...
**The obligation that follows** is that the import surface is the product boundary, and every import is API owed permanently. It is kept minimal and specified. See `wasi/DECISIONS.md` D9.
...
**This port is what the engine will do for you.** Compatibility is a **preference**, and its bar is that *most* games written for 11.5 / Lua 5.1 play — either directly, or after a minimal, predictable patch **love.wasm applies automatically**. An auto-shim is therefore a deliverable, not a purity violation: desktop LÖVE runs Lua 5.1 through LuaJIT, where `unpack`, `table.getn` and `math.atan2` all exist and where `newFont(path, 4.5)` works, so restoring them moves this build *toward* desktop behavior rather than away from it. It is deliberately deferred to late in development, so it is not rebuilt repeatedly against a moving engine (`wasi/DECISIONS.md` D14). Until it lands, the port below is what a game needs.
