...
**Status: the LÖVE core boots in a real browser, and now plays and captures audio.** Build-order steps 0–3 and 5 are done and browser-verified (see the build order below): real `love.math` and `love.data` run under this repo's frame pump over the pinned lua-wasi source drop, LÖVE 12's own main-loop function executes to its documented stop-line at the `love.filesystem` seam, and `love.audio` — playback and microphone, on a WebAudio backend — is witnessed recovering a test tone headless (step 5 landed ahead of the step-4 graphics seam). The tree stays upstream-shaped — edits to shared engine source are two small seams: the platform seam in `common/config.h` (a three-line `__wasi__` → `LOVE_WASI` guard plus one clause added to the platform sanity-check), and a guarded audio-backend-selection seam in `src/modules/audio/wrap_Audio.cpp` (OpenAL/WebAudio/null selection, byte-unchanged for default builds, offered upstream). Outside `wasi/`, the fork adds only two CI workflows (`.github/workflows/witness.yml` and `.github/workflows/publish-sysroot.yml`), the interactive-session bring-up hook and settings under `.claude/`, a few `.gitignore` lines, and the governance docs (`AGENTS.md`, `CLAUDE.md`).
