...
0. Toolchain bring-up: wasm-EH libc++/libc++abi built and witnessed by a trivial typed-catch program. — **Done, browser-verified (2026-07-06):** `wasi/toolchain/build-libcxx-eh.sh` builds both from LLVM 20.1.2 source (plus the `Unwind-wasm.c` personality shim); `wasi/witness/eh-typed-catch.cpp` — typed catch via a base class, `what()` intact, a carried payload surviving the throw, a non-matching `catch` skipped so the outer typed handler wins, and a destructor run during unwind — **PASSes in Chromium 141** through `wasi/witness/run-browser.mjs` (driven by our own WASI preview1 shim — `wasi/host/wasi-shim.mjs`, one module shared by all three browser witnesses, the seed of the runtime host) and under `node:wasi`. The whole check is one saved command, `wasi/witness/run.sh` (both legs must pass), re-run by CI on every push to `wasi` (`.github/workflows/witness.yml`).
