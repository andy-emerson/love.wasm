...
// A Source over one host playback voice (a WebAudio node). The engine hands PCM
// through wa_source_queue and control through wa_source_play/stop/gain; the host
// mixes and resamples. Spatialization (position/cone/distance) is stored but not
// yet routed — HRTF is deferred (wasi/audio/DESIGN.md), so those are inert like
// the null backend until a device-agnostic spatializer is designed.
...
private:
	...
	// Lazily create the host voice (needs a live host; -1 until then / if none).
	int ensureHandle();
