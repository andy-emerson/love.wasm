#!/usr/bin/env bash
# Build the step-6.7 artifact: the EMBEDDING CONTRACT. Source-identical to the
# 6.2 build (build-fs2.sh) — the write path adds NO new module or TU: it lives
# entirely in fs-backend.cpp (already linked here) on new love_fs write imports
# the host fulfils, and the reload/invalidate primitive lives in the pump
# (wasi/pump/pump.cpp, also already linked). So this is build-fs2.sh with a
# different OUT; it links:
#
...
echo "built $OUT (LÖVE core + real love.filesystem read+write on love_fs seam + pump_invalidate reload; external EH confirmed)"
